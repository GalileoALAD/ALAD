var ALADToken = artifacts.require("ALADToken");

contract('ALADToken', function(accounts) {
  it("should not be able to start ico sale when not called by owner.", function() {
    console.log("\n" + "-".repeat(100) + "\n");
    var ALAD;
    var target;
    return ALADToken.deployed().then(function(instance) {
      ALAD = instance;
      console.log("ALAD:", ALAD.address);
      return ALAD.target.call({from: accounts[1]});
    }).then(function(t){
      target = accounts[1];
      web3.eth.sendTransaction({from: accounts[1], to: ALAD.address, value: web3.toWei(1) })
      return web3.eth.sendTransaction({from: accounts[1], to: target, value: web3.toWei(1) })
    }).then(function(tx) {
      console.log("tx:", tx);
      console.log("blockNumber:", web3.eth.blockNumber);
      return ALAD.start(100, {from: accounts[1]});
    }).then(function(tx) {
      console.log("tx:", tx);
      if (tx.logs) {
  ...
  it("should not allow create tokens before it starts", function() {
    ...
    return ALADToken.deployed().then(function(instance) {
      ...
      return web3.eth.sendTransaction({from: accounts[1], to: ALAD.address, value: web3.toWei(1) });
  ...
  it("should be able to start ico sale when called by owner.", function() {
    ...
    return ALADToken.deployed().then(function(instance) {
      ALAD = instance;
      return ALAD.target.call();
    }).then(function(t){
      target = t;
      return ALAD.start(10, {from: target});
    }).then(function(tx) {
      console.log("tx: ", tx);
      if (tx.logs) {
        for (var i = 0; i < tx.logs.length; i++) {
  ...
  it("should be able to create ALAD tokens after sale starts", function() {
    console.log("\n" + "-".repeat(100) + "\n");
    var ALAD;
    var target;
    var externalTxHash;
    return ALADToken.deployed().then(function(instance) {
      ...
      target = t;
      return web3.eth.sendTransaction({from: accounts[1], to: ALAD.address, value: web3.toWei(1), gas: 500000 });
    }).then(function(tx) {
      console.log("tx:", tx);
      return ALAD.balanceOf(accounts[1], {from: accounts[1]});
    }).then(function(bal) {
      console.log("bal: ", bal.toNumber());
      assert.equal(bal.toNumber(), web3.toWei(6000), "no ALAD token Transfer event found");
    });
  });
  ...
  //     return ALAD.computeTokenAmount(web3.toWei(1));
  ...
  //     return ALAD.computeTokenAmount(web3.toWei(2.19));
  ...
  //     return ALAD.computeTokenAmount(web3.toWei(3.04598));
  ...
  //     return ALAD.computeTokenAmount(web3.toWei(1237.472));
  ...
  //     return ALAD.computeTokenAmount(web3.toWei(0.01));
...
//     }).then(function(t){
//       target = t;
//       return ALAD.ethGoalPerPhase.call({from: accounts[1]});
//     }).then(function(goal){
...
//     }).then(function(tx) {
//       console.log("txHash 1: ", tx);
//       return ALAD.computeTokenAmount(web3.toWei(12.19), {from: accounts[1]});
//     }).then(function(amount1){
//       console.log("amount1:", amount1);
...
//     }).then(function(tx) {
//       console.log("txHash 1: ", tx);
//       return ALAD.tokenAmountForOwner({from: accounts[1]});
//     }).then(function(amount1){
//       console.log("amount1:", amount1);
